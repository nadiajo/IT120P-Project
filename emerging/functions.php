...
function getUserData($userId) {
    $conn = db_Connect();
    $sql = "SELECT full_name, email_address, phone_number, street_address FROM users WHERE id = ?";
...
function updateUserData($userId, $fullName, $email, $phoneNumber, $streetAddress) {
    $conn = db_Connect();
    $sql = "UPDATE users SET full_name=?, email_address=?, phone_number=?, street_address=? WHERE id=?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssssi", $fullName, $email, $phoneNumber, $streetAddress, $userId);
    $stmt->execute();
...
function getUserPasswordHash($userId) {
    // Database connection
    $conn = db_connect();  // Assuming db_connect() is your function to get a database connection
...
function updateUserPassword($userId, $newPasswordHash) {
    // Database connection
    $conn = db_connect();  // Re-use your existing database connection function
